...
if (Meteor.isClient) {
 ...
 Template.brands.helpers({
  ...
  // uploads:function(){
  //     var file = _.pluck(Brands.find({_id: "Hd3MQbByRp6M5CqzY"}).fetch(), 'fileId');
  //     var media = _.pluck(Uploads.find({_id: file}).fetch(), 'url');
  //     console.log(media) 

  // },
  // media:function(){
  //       var id = Brands.findOne({_id:this._id});
...
Template.brands.events({
  'click .edit': function () {
    // alert(this._id);
    window.location = Router.url('brands')+'/'+ this._id;
...
Template.ShowBrands.rendered = function() {
