...
if (Meteor.isClient) {
 ...
 Template.stock.helpers({
  ...
  // uploads:function(){
  //     var file = _.pluck(Stock.find({_id: "Hd3MQbByRp6M5CqzY"}).fetch(), 'fileId');
  //     var media = _.pluck(Uploads.find({_id: file}).fetch(), 'url');
  //     console.log(media) 

  // },
  // media:function(){
  //       var id = Stock.findOne({_id:this._id});
...
Template.stock.events({
  'click .edit': function () {
    // alert(this._id);
    window.location = Router.url('stock')+'/'+ this._id;
...
Template.ShowStock.rendered = function() {
